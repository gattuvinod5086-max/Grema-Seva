...
function createDemoStore(district: string, mandal: string, village: string): EmergencyStore {
  ...
  const policeStations: PoliceStationRecord[] = [
    {
      id: psId,
      name: "Bhongir Town Police Station (DEMO)",
      district,
      mandal: "Bhongir",
      address: "Near Bhongir Bus Stand — DEMO ADDRESS ONLY",
      phone: DEMO_PHONE,
      alternate_phone: "9000000001",
      latitude: 17.5155,
      longitude: 78.8856,
      jurisdiction: "Bhongir mandal and surrounding villages",
      is_active: true,
      verified_at: null,
      source: "DEMO DATA — replace with verified Telangana Police directory before production",
      isDemoData: true,
    },
  ];
  ...
  const villagePoliceMapping: VillagePoliceMapping[] = [
    ...
    { id: "vpm-2", district, mandal: "Bhongir", village: "Bhongir City", police_station_id: psId },
    { id: "vpm-3", district, mandal: "Bhongir", village: "Raigiri", police_station_id: psId },
    { id: "vpm-4", district, mandal: "Bhongir", village: "Anantharam", police_station_id: psId },
  ];

  const contacts: EmergencyContactRecord[] = [
    {
      id: "ec-phc-1",
      name: `${mandal} Primary Health Centre (DEMO)`,
      service_type: "PHC",
      description: "Primary health centre — DEMO facility record",
      phone: DEMO_PHONE,
      district,
      mandal,
      village,
      address: "Near Mandal HQ — DEMO",
      latitude: 17.52,
      longitude: 78.88,
      scope: "VILLAGE",
      is_emergency: true,
      is_active: true,
      verified_at: null,
      source: "DEMO DATA — load from official health facility registry",
      isDemoData: true,
    },
    {
      id: "ec-hosp-1",
      name: "District Government Hospital Bhongir (DEMO)",
      service_type: "HOSPITAL",
      description: "Government hospital — DEMO",
      phone: "9000000002",
      district,
      mandal: "Bhongir",
      address: "Bhongir — DEMO",
      latitude: 17.518,
      longitude: 78.89,
      scope: "DISTRICT",
      is_emergency: true,
    ...
    {
      id: "ec-fire-1",
      name: "Bhongir Fire Station (DEMO)",
      service_type: "FIRE",
      phone: "9000000003",
      district,
      mandal: "Bhongir",
      address: "Fire Station Road — DEMO",
      latitude: 17.516,
      longitude: 78.884,
      scope: "MANDAL",
      is_emergency: true,
      is_active: true,
      verified_at: null,
      source: "DEMO DATA — Telangana Fire Services",
      isDemoData: true,
    },
    {
      id: "ec-elec-1",
      name: "TGSPDCL Emergency (DEMO regional)",
      service_type: "ELECTRICITY",
      description: "Electricity emergency — configure regional TGSPDCL helpline",
      phone: "1912",
      district,
      scope: "STATE",
      is_emergency: true,
      ...
      verified_by: "demo-config",
      source: "TGSPDCL — verify current regional number before production",
      isDemoData: true,
    },
    {
      id: "ec-water-1",
      name: "Mission Bhagiratha / Water Emergency (DEMO)",
      service_type: "WATER",
      description: "Water supply emergency contact",
      phone: DEMO_PHONE,
      district,
      mandal,
      village,
      ...
      is_active: true,
      verified_at: null,
      source: "DEMO DATA — configure mandal water authority",
      isDemoData: true,
    },
    {
      id: "ec-panch-1",
      name: "Gram Panchayat Office (DEMO)",
...
export const EmergencyMockDB = {
  ...
  seedDemo: (village: string, district: string, mandal: string) => {
    ...
    // Ensure current village has police mapping and village-scoped contacts
  ...
  savePoliceStation: (ps: PoliceStationRecord) => {
    const store = readStore();
    const idx = store.policeStations.findIndex((p) => String(p.id) === String(ps.id));
    if (idx >= 0) store.policeStations[idx] = ps;
    else store.policeStations.push(ps);
    writeStore(store);
  },

  saveMapping: (mapping: VillagePoliceMapping) => {
    const store = readStore();
    const idx = store.villagePoliceMapping.findIndex((m) => String(m.id) === String(mapping.id));
    if (idx >= 0) store.villagePoliceMapping[idx] = mapping;
    else store.villagePoliceMapping.push(mapping);
    writeStore(store);
  },
};
